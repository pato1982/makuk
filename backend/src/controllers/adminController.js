  ...
  try {
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      await conn.query(
        ...
        [p.nombre, p.categoria, p.imagen, p.imagen2 || '', p.imagen3 || '', p.precioActual || 0, p.precioAnterior || 0, p.descripcion, p.deliveryTime || '', p.destacado ? 1 : 0, i + 1, p.imagePosX ?? 50, p.imagePosY ?? 50, p.imageZoom ?? 1, p.image2PosX ?? 50, p.image2PosY ?? 50, p.image2Zoom ?? 1, p.image3PosX ?? 50, p.image3PosY ?? 50, p.image3Zoom ?? 1]
      );
    }
